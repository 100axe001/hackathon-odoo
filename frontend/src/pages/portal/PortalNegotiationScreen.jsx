...
export function PortalNegotiationScreen() {
  ...
  const submitRequest = async () => {
    try {
      ...
      setShowBanner(true);
      loadPortalQuotation(id).then(setDetail);
    } catch {
  ...
  const confirm = async () => {
    try {
      ...
      setBanner("Could not confirm. Please try again.");
      setShowBanner(true);
    }
  };
      ...
      <Card className="mb-6">
        ...
        >
          ...
          <span className="text-sm tabular-nums" style={{ color: C.muted }}>
            Total ${(detail.total ?? 0).toLocaleString()}
          </span>
        </div>
          ...
          <tbody>
            {(detail.lines ?? []).map((l) => (
              <Tr key={l.id}>
                <Td>{l.product}</Td>
                <Td right>{l.qty}</Td>
                <Td right>${l.price.toLocaleString()}</Td>
